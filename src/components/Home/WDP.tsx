...
export const WDP = () => {
  ...
  const wExplosionRef = useRef<HTMLDivElement>(null);
  const dExplosionRef = useRef<HTMLDivElement>(null);
  const pExplosionRef = useRef<HTMLDivElement>(null);
  const mExplosionRef = useRef<HTMLDivElement>(null);
  const wTextRef = useRef<HTMLSpanElement>(null);
  const mTextRef = useRef<HTMLSpanElement>(null);
  const dTextRef = useRef<HTMLSpanElement>(null);
  ...
  const fullWords: Record<string, string> = {
    w: "eb",
    m: "obile",
    d: "esign",
    p: "rint",
  ...
  const explosionImages: Record<string, string[]> = {
    w: [
      "/objects/obj-11.png",
      "/objects/obj-11.png",
      "/objects/obj-11.png",
  ...
  useGSAP(
    () => {
      let activeAnimations: Record<string, number | null> = {
        w: null,
        d: null,
        p: null,
        m: null,
      ...
      let clickTimeouts: Record<string, NodeJS.Timeout | null> = {
        w: null,
        d: null,
        p: null,
        m: null,
      ...
      const createParticles = (
        explosionRef: React.RefObject<HTMLDivElement | null>,
        imagePaths: string[]
      ) => {
      ...
      const wLetter = containerRef.current?.querySelector(
        '[data-letter="w"]'
      ) as HTMLElement;
      const dLetter = containerRef.current?.querySelector(
        '[data-letter="d"]'
      ) as HTMLElement;
      const pLetter = containerRef.current?.querySelector(
        '[data-letter="p"]'
      ) as HTMLElement;
      const mLetter = containerRef.current?.querySelector(
        '[data-letter="m"]'
      ) as HTMLElement;
