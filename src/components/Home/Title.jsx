...
export const Title = () => {
  ...
  useGSAP(() => {
    if (helloRef.current && wereRef.current && !isLoaded) {
      ...
      const helloChars = helloText.chars;
      const wereChars = wereText.chars;
  ...
  const handleHelloHover = (isEntering) => {
    if (!helloRef.current || !wereRef.current) return;

    const helloChars = helloRef.current.querySelectorAll(".char");
    const wereChars = wereRef.current.querySelectorAll(".char");
    const allChars = [...helloChars, ...wereChars];
  ...
  useGSAP(() => {
    if (creativeRef.current) {
      ...
      const chars = container.querySelectorAll(".letter-container");
