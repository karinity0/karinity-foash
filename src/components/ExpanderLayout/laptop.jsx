...
import { LogoKarinity } from "../Menu/logoKarinity";
...
export const MacbookScroll = ({ src, showGradient, title, badge }) => {
  ...
  useGSAP(() => {
    if (!ref.current || !titleRef.current || !lidRef.current) return;
    ...
    const scrollTrigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        const translate = gsap.utils.mapRange(0, 1, 0, 1500, progress);

        // Rotate calculation with multiple breakpoints
        // At progress 0.1: -28, at 0.12: -28, at 0.3: 0
        let rotate = -28;
        if (progress < 0.1) {
          rotate = -28;
        } else if (progress >= 0.1 && progress < 0.12) {
          rotate = -28;
        } else if (progress >= 0.12 && progress <= 0.3) {
          rotate = gsap.utils.mapRange(0.12, 0.3, -28, 0, progress);
        } else {
          rotate = 0;
        }

        const textTransform = gsap.utils.mapRange(0, 0.3, 0, 100, progress);
        const textOpacity = gsap.utils.mapRange(0, 0.2, 1, 0, progress);
  ...
  return (
    <div
      ref={ref}
      className="flex min-h-[50vh] shrink-0 scale-[0.35] transform flex-col items-center justify-start py-0 perspective-midrange sm:scale-50 md:scale-75 md:py-40"
    >
      <h2
        ref={titleRef}
...
export const Lid = React.forwardRef(({ src }, ref) => {
  return (
    <div className="relative perspective-midrange">
      ...
      >
        <div
          ...
          className="absolute inset-0 flex items-center justify-center rounded-lg bg-[#ffffff]"
        >
      ...
      {/* TODO: Add the image to the laptop */}
      {/* <div
        ref={ref}
