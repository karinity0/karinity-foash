"use client";
import { trimSvgViewBox } from "@/lib/utils";
...
export const LogoKarinity = () => {
  ...
  const karinityRef = useRef<SVGSVGElement>(null);
  const mouthAnimationRef = useRef<gsap.core.Tween | null>(null);
  ...
  useGSAP(
    () => {
      ...
      const letters = gsap.utils.toArray<SVGPathElement | SVGPolygonElement>(
      ...
      const originalPacManXPosition = pacmanGroup?.getAttribute("x");
      ...
      const createCrumbs = (letterElement: SVGPathElement | SVGPolygonElement, letterBounds: LetterPosition) => {
        ...
        for (let i = 0; i < numCrumbs; i++) {
          ...
          crumb.setAttribute("x", letterBounds.centerX.toString());
          crumb.setAttribute("y", letterBounds.centerY.toString());
          crumb.setAttribute("font-size", gsap.utils.random(40, 50).toString());
      ...
      const letterPositions: LetterPosition[] = letters.map((letter) => {
      ...
      let mouthAnimation: gsap.core.Tween | null = null;
      ...
      if (mouthPath && originalMouthPath) {
        ...
        const mouthState: MouthState = { ...closedMouth };
      ...
      const tl = gsap.timeline({
        repeat: 0,
        onStart: () => {
          // Hide nav-toggle when animation starts (only on mobile)
        ...
        onComplete: () => {
          ...
          karinityRef.current?.setAttribute(
            "viewBox",
            "103.77 443.22 167.57 193.56"
          );
          if (containerRef.current) {
            containerRef.current.style.width = "40px";
          }

          // Show nav-toggle when animation completes (only on mobile)
          if (window.innerWidth < 1000) {
            const navToggle = document.querySelector(".nav-toggle");
            if (navToggle) {
              gsap.to(navToggle, {
      ...
      // Store timeline and mouth animation refs for handleMouseEnter
      timelineRef.current = tl;
      mouthAnimationRef.current = mouthAnimation;
  ...
  const handleMouseEnter = () => {
    if (timelineRef.current && !timelineRef.current.isActive()) {
      ...
      if (karinityRef.current && containerRef.current) {
        karinityRef.current.setAttribute(
          "viewBox",
          "103.77 443.22 872.46 193.56"
        );
        containerRef.current.style.width = "";
      }

      // Restart mouth animation if it exists
      if (mouthAnimationRef.current) {
        mouthAnimationRef.current.restart();
      }

      // Restart the timeline (onStart callback will hide nav-toggle)
  ...
  return (
    <div
      className="h-auto w-32 md:w-40 flex items-center justify-center justify-self-end self-center"
      ref={containerRef}
    >
      <svg
        version="1.1"
      ...
      >
        <g>
          ...
          <g>
            <g>
              <path
                d="M175.97,443.22h-49.59v33.28c13.15-13.24,30.36-22.43,49.59-25.5V443.22z"
                fill="white"
              ></path>
              <polygon
                className="st0"
            ...
            <path
              fill="white"
              d="M153.09,480.05c-0.59,0.36-1.17,0.73-1.74,1.12c-3.96,2.63-7.64,5.65-10.98,9.01c-5.74,5.75-10.48,12.47-13.98,19.89v8.37             l21.42,21.54l-0.02,0.01l-7.12,7.12l-14.28,14.28v75.36h49.29v-0.06h0.01v-0.03l0.28,0.09V470.9             C167.73,472.6,160.01,475.74,153.09,480.05z"
            ></path>
          </g>
        </g>
      </svg>
