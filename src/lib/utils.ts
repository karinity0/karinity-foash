...
interface TrimSvgViewBoxOptions {
  width?: number | null;
  height?: number | null;
  padding?: number;
  removeAttrs?: boolean;
...
interface TrimSvgViewBoxResult {
  svg: SVGElement;
  outerHTML: string;
  viewBox: string | null;
}

export function trimSvgViewBox(
  svgEl: SVGElement,
  options: TrimSvgViewBoxOptions = {}
): TrimSvgViewBoxResult {
  ...
  (function walk(node: Element) {
    for (let child of Array.from(node.children)) {
      if (isSkippable(child)) {
        ...
        if (child.children && child.children.length) walk(child as Element);
        continue;
      }
  ...
  if (candidates.length === 0) {
    for (let c of Array.from(svgEl.children)) {
      if (!isSkippable(c)) candidates.push(c as Element);
    }
    if (candidates.length === 0) {
  ...
  candidates.forEach((el) => {
    try {
      ...
      const svgElElement = el as SVGGraphicsElement;
      if (!svgElElement.getBBox) return;
      const bbox = svgElElement.getBBox();
      ...
      const svgRoot = (svgEl.ownerSVGElement || svgEl) as SVGSVGElement;
      const ctm = svgElElement.getCTM
        ? svgElElement.getCTM() || svgRoot.createSVGMatrix()
